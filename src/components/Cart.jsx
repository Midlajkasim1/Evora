...
export default function Cart({ isOpen, onClose, items, onRemove, onUpdateQty }) {
  ...
  return (
    <>
      ...
      <aside className={`cart-drawer${isOpen ? ' cart-drawer--open' : ''}`} aria-label="Shopping cart" role="dialog">
        <div className="cart-drawer__header">
          <h3>Your Cart <span className="cart-drawer__count">({items.length})</span></h3>
                ...
                <li key={item.id} className="cart-item">
                  ...
                  <div className="cart-item__info">
                    ...
                    <div className="cart-item__controls">
                      <button className="cart-qty-btn" onClick={() => onUpdateQty(item.id, item.qty - 1)} aria-label="Decrease quantity">−</button>
                      <span className="cart-qty">{item.qty}</span>
                      <button className="cart-qty-btn" onClick={() => onUpdateQty(item.id, item.qty + 1)} aria-label="Increase quantity">+</button>
                    </div>
                  </div>
                  <button className="cart-item__remove" onClick={() => onRemove(item.id)} aria-label={`Remove ${item.name}`}>
